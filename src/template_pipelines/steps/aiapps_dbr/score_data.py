...
class ScoreDataStep(Step):
    ...
    def run(self):
        ...
        # Write files under insights directory

        with TemporaryDirectory() as temp_dir:
            # Write json file for chart data options
            json_file_name = self.config["aiapps_outputs_files"]["file_1"]["name"]
            file_path = os.path.join(temp_dir, json_file_name)

            self.outputs["results_options_json"] = write_json_file(graph_options, file_path)
            ...
            self.outputs["confusion_matrix"] = write_json_file(new_data, file_path)
            ...
            self.outputs["precision_recall_curve"] = write_json_file(new_data, file_path)
            ...
            self.outputs["roc_curve"] = write_json_file(new_data, file_path)

            # Write csv file for table data
            csv_file_name = self.config["aiapps_outputs_files"]["file_5"]["name"]
            file_path = os.path.join(temp_dir, csv_file_name)

            self.outputs["validation_predictions_csv"] = write_csv_file(results_table, file_path)

            # Write json file for validation predictions bar
            json_file_name = self.config["aiapps_outputs_files"]["file_6"]["name"]
            file_path = os.path.join(temp_dir, json_file_name)

            self.outputs["validation_predictions_json"] = write_json_file(data_dict, file_path)

            self.outputs["aiapps_outputs_insights"] = temp_dir
        ...
        if self.runtime_parameters.get("config_file_1") not in ["None", None, ""]:
            file_paths.append(self.inputs["config_file_1_input"])

        if self.runtime_parameters.get("run_file_1") not in ["None", None, ""]:
            file_paths.append(self.inputs["run_file_1_input"])

        with TemporaryDirectory() as temp_dir:
            [shutil.copy(file_path, temp_dir) for file_path in file_paths]

            self.outputs["aiapps_outputs_uploads"] = temp_dir
