...
This validates transformed data from feature_creation based on expectations
...
# Define DataValidationAfterFeatureCreation class
# and inherit properties from pyrogai DqStep class
class DataValidationAfterFeatureCreationDqStep(DqStep, suffix="DqStep"):  # type: ignore
    """Data Validation After Freature Creation step."""

    # Pyrogai executes code defined under run method
    def run(self):
        """Run Data Validation After Feature Creation step."""
        ...
        # Find a GE critical.html and set it to ioslot to visualize results
        ge_doc_path = next(self.ioctx.get_fns(f"ge_dq_datadocs/{self.step_name}/**/critical.html"))
