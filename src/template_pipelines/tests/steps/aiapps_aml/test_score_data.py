"""Unit tests for template_pipelines/steps/aiapps_aml/score_data.py."""
from unittest.mock import MagicMock, patch

import pandas as pd
import pytest

from template_pipelines.steps.aiapps_aml.score_data import ScoreDataStep
...
@patch("template_pipelines.steps.aiapps_aml.score_data.write_json_file")
@patch("template_pipelines.steps.aiapps_aml.score_data.write_csv_file")
@patch("template_pipelines.steps.aiapps_aml.score_data.pickle")
@patch("template_pipelines.steps.aiapps_aml.score_data.open")
@patch("template_pipelines.steps.aiapps_aml.score_data.pd")
def test_score_data_step_run(
    ...
    with patch(
        "template_pipelines.steps.aiapps_aml.score_data.score_data",
        return_value=(MagicMock(), MagicMock()),
    ) as mock_score_data, patch(
        "template_pipelines.steps.aiapps_aml.score_data.generate_results_table",
        return_value=mock_result_table,
    ) as mock_generate_results_table, patch(
        "template_pipelines.steps.aiapps_aml.score_data.plot_confusion_matrix",
        return_value=MagicMock(),
    ) as mock_plot_confusion_matrix, patch(
        "template_pipelines.steps.aiapps_aml.score_data.plot_precision_recall_curve",
        return_value=MagicMock(),
    ) as mock_plot_precision_recall_curve, patch(
        "template_pipelines.steps.aiapps_aml.score_data.plot_roc_curve", return_value=MagicMock()
